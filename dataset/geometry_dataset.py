...
class GeometryPartDataset(Dataset):
    ...
    def _read_data(self, data_fn):
        ...
        if os.path.exists(os.path.join(self.data_dir, 'geometry_dataset_primary_datalist_'+data_fn)):
            with open(os.path.join(self.data_dir, 'geometry_dataset_primary_datalist_'+data_fn), 'rb') as f:
                print('load data_list from existing file')
        ...
        with open(os.path.join(self.data_dir, 'geometry_dataset_primary_datalist_'+data_fn), 'wb') as f:
    ...
    def _get_pcs(self, data_folder):
        ...
        for mesh in meshes:
            if type(mesh) == list:
                print(data_folder, ' ', mesh_files[i])
            pcs.append(trimesh.sample.sample_surface(mesh, self.num_points)[0])
            i = i + 1
        # pcs = [
