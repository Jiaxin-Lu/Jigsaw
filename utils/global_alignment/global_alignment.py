...
def global_alignment(v_num, edges, transformations, uncertainty, method='gtsam', verbose=False):
    """
        Input:
    ...
    if method == 'gtsam':
        global_pose_results, success = shonan_averaging(v_num + 1, edges, transformations, uncertainty, verbose=verbose)
        if success == 0:
